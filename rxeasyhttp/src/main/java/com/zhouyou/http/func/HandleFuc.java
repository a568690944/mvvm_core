...
public class HandleFuc<T> implements Function<ApiResult<T>, T> {
    ...
    public T apply(@NonNull ApiResult<T> tApiResult) throws Exception {
        if (ApiException.isOk(tApiResult)) {
            return tApiResult.getContent();// == null ? Optional.ofNullable(tApiResult.getData()).orElse(null) : tApiResult.getData();
        } else {
            throw new ServerException(tApiResult.getRetCode(), tApiResult.getRetMsg());
        }
    }
}
